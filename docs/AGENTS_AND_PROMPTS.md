...
This document describes the architecture, implementation paths, prompt templates, injected context fields, and output schemas for all AI agents in the REROUTE.ai platform. It also outlines the Dynamic Risk Score Allocation and Background Alerting systems.

---

## 🗺️ Multi-Agent System Overview

The REROUTE.ai backend leverages a multi-agent structure powered by Google Gemini and Google's IQ AI ADK framework. The system consists of specialized agents coordinating to analyze supply chain networks, forecast disruptions, simulate scenarios, assess financial impacts, and recommend mitigation strategies.
...
- **File Location**: [strategy/route.ts](file:///c:/reroute.ai/app/api/agent/strategy/route.ts)
...
- **File Location**: [forecast/route.ts](file:///c:/reroute.ai/app/api/agent/forecast/route.ts)
...
- **File Location**: [impact/route.ts](file:///c:/reroute.ai/app/api/agent/impact/route.ts)
...
- **File Location**: [scenario/route.ts](file:///c:/reroute.ai/app/api/agent/scenario/route.ts)
...
- **File Location**: [orchestrator/route.ts](file:///c:/reroute.ai/app/api/agent/orchestrator/route.ts)
...
- **File Location**: [news/route.ts](file:///c:/reroute.ai/app/api/news/route.ts) & [news-polling/route.ts](file:///c:/reroute.ai/app/api/agent/news-polling/route.ts)
...
- **File Location**: [info/route.ts](file:///c:/reroute.ai/app/api/agent/info/route.ts)
...
- **File Location**: [automated-alerts/route.ts](file:///c:/reroute.ai/app/api/agent/automated-alerts/route.ts)
...
- **File Locations**: 
  - [suggestions/route.ts](file:///c:/reroute.ai/app/api/suggestions/route.ts) (System-level suggestions payload)
  - [useAISuggestions.ts](file:///c:/reroute.ai/components/digital-twin/layout/left-panel/assistant/useAISuggestions.ts) (Canvas general suggestion chips & chat autocompletion chips)
...
To prevent generic or static risk assessments, REROUTE.ai allocates real-time risk scores using external Tavily search queries targeted at physical node locations.
