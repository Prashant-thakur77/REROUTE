...
export default function DigitalTwinCard({ twin }: DigitalTwinCardProps) {
  ...
  return (
    <Link href={`/digital-twin/view/${twin.id}`} className="block h-full">
      <Card className="hover:shadow-lg hover:scale-[1.02] transition-all duration-300 h-full border shadow-md group relative overflow-visible cursor-pointer">
        ...
        <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0 z-50">
          <div className="bg-card/90 rounded-full p-2 shadow-sm">
            <ArrowRight className="h-4 w-4 text-muted-foreground" />
          </div>
        </div>
        
        <CardHeader className="relative z-10 pb-3">
          <CardTitle className="text-foreground group-hover:text-muted-foreground transition-colors duration-200 flex items-start justify-between gap-2 pr-10 min-w-0">
            <span className="line-clamp-2 break-words min-w-0">{twin.name}</span>
          </CardTitle>
          <CardDescription className="text-muted-foreground group-hover:text-foreground transition-colors duration-200 line-clamp-2 break-words">
            {twin.description}
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="relative z-10 pt-0">
          <div className="flex flex-wrap gap-2">
            {twin.tags.map((tag, index) => {
              ...
              const tagClass = isRiskTag ? getRiskColor(tag) : 'bg-muted text-foreground border border-border shadow-sm';
              const isDateTag = /\w{3}-\d{4}/.test(tag); // Matches format like "Jun-2024"
              
              return (
                <Badge
                  key={tag}
                  className={`${tagClass} transition-all duration-200 border-0 font-medium text-xs px-2 py-1 inline-flex items-center gap-1 max-w-full break-words`}
                >
